...
$columnWidths = [6,40,20,20,15,15,18,14,15,18,18];

// Table header
$pdf->Cell($columnWidths[0], 7, 'NO', 1, 0, 'C', true);
$pdf->Cell($columnWidths[1], 7, 'Name', 1, 0, 'C', true);
$pdf->Cell($columnWidths[2], 7, 'Lone date', 1, 0, 'C', true);
$pdf->Cell($columnWidths[3], 7, 'Due date', 1, 0, 'C', true);
$pdf->Cell($columnWidths[4], 7, 'Rental', 1, 0, 'C', true);
$pdf->Cell($columnWidths[5], 7, 'Loan amo', 1, 0, 'C', true);
$pdf->Cell($columnWidths[6], 7, 'Agree val', 1, 0, 'C', true);
$pdf->Cell($columnWidths[7], 7, 'No rental', 1, 0, 'C', true);
$pdf->Cell($columnWidths[8], 7, 'passed dy', 1, 0, 'C', true);
...
$result = mysqli_query($conn, "SELECT name, amount, total_arrears,due_date,rental,agree_value,no_rental,total_payments,no_pay,days_passed,lone_date, status FROM borrowers ORDER BY id ASC");
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell($columnWidths[2], 5, $row['lone_date'], 1, 0, 'C', true);
    $pdf->Cell($columnWidths[3], 5, $row['due_date'], 1, 0, 'C', true);
    $pdf->Cell($columnWidths[4], 5, $row['rental'], 1, 0, 'C', true);
    $pdf->Cell($columnWidths[5], 5, number_format($row['amount'], 2), 1, 0, 'C', true);
    $pdf->Cell($columnWidths[6], 5, number_format($row['agree_value'], 2), 1, 0, 'C', true);
    $pdf->Cell($columnWidths[7], 5, $row['no_rental'], 1, 0, 'C', true);
    $pdf->Cell($columnWidths[8], 5, $row['days_passed'], 1, 0, 'C', true);
