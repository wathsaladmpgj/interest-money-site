...
<head>
    <meta charset="UTF-8">
    <meta http-equiv="refresh" content="100"> <!-- Refresh every 5 seconds -->
...
<form method="post">
        ...
        <?php
        // Populate the year dropdown with available years in monthly_details table
...
while ($monthly_interest = $result_monthly->fetch_assoc()) {
    ...
    if ($monthly_date >= $updated_date) {
        ...
        // Use the stored values in `monthly_details` for previous months
    ...
    $interest_sum = $interest1 + $interest2 + $interest3 + $interest4+$monthly_interest['capital_received'];
